...
- ניסיון ניקוי 18.8.2026: אומתו 52 ברנצ'ים בטוחים למחיקה (47 מוזגים + 5 שתוכנם כבר ב-main בדרך אחרת) בנוסף ל-4 ברנצ'ים ישנים של אתר הבלונים שאושרו למחיקה — **המחיקה בפועל לא בוצעה** בגלל המגבלה למעלה. הרשימה המלאה (56 שמות ברנץ') תועדה בבקשת הניקוי מ-18.8; מישהו עם הרשאות כתיבה צריך להריץ אותה.
- 3 ברנצ'ים נוספים (`dobble-game-generator-joiLU`, `hoshaya-balloon-site-DFjqp`, `hoshaya-site-architecture-bxc7tt`) **סומנו בטעות כ"מוזגים"** בבקשת הניקוי — בפועל אין להם merge-base משותף עם `main` בכלל (תוכן זר לגמרי, אתר בלונים, כנראה מ-push בטעות לריפו הזה מסשן multi-repo). לא נמחקו, לא אושרו — דורש בדיקה/אישור נפרד לפני מחיקה.
