...
## סקירה
> 📋 **לפני כל עבודה — לקרוא את `מרשם.md` בשורש הריפו.**
> הוא מחזיק את מצב כל המשימות הפתוחות והכללים לעבודה מקבילה.
> בסיום כל משימה — לעדכן אותו באותו PR.
...
- ספריות: jsPDF, JSZip, Supabase (מ-CDN) · **heic2any ו-UTIF מגיעות מ-`vendor/` בריפו עצמו** (ראו "פורמטים" למטה).
...
**כלל cache busting — כל שינוי בקובץ תחת `js/` מחייב העלאת ה-`?v=N` בתגית ה-`<script>` המתאימה ב-`dobble.html`, באותו commit.** בלי זה הדפדפן יגיש את הגרסה הישנה מהמטמון. קרה בפועל: אחרי מיזוג PR #59 (תיקן באג ב-`dobble.html` וגם ב-`js/ai-symbols.js`) התיקון ב-HTML נכנס מיד והתיקון ב-JS לא — שתי כתובות מטמון נפרדות, והדפדפן החזיק את ה-JS הישן. ההשלכה חמורה מהבאג עצמו: בדיקה ידנית שרואה התנהגות ישנה אחרי תיקון נראית כמו כישלון התיקון, כשבפועל הוא פשוט לא נטען.
**כלל idbLoad — פונקציה שעושה `push` ל-`S.assets` חייבת שומר ריצה-אחת.**
מקור: המאגר הוכפל אחרי רענון. `idbLoad()` מוסיף ל-`S.assets` ולא מאפס אותו (בניגוד ל-`applyDesignToState()` שכן מאפס), ושני נתיבים קוראים לו — האתחול (`sessionStorage.dobble_unlocked==='1'`) ו-`window._afterUnlock`, והאחרון נקרא גם מ-`unlockFromSession` שרץ על `onAuthStateChange('SIGNED_IN')`. אצל משתמש מחובר שניהם רצו: כל תמונה פעמיים, נשמרה כפולה חזרה ל-IDB, והוכפלה שוב בטעינה הבאה. היום `idbLoad` מחזיר הבטחה יחידה (`_idbLoadOnce`) ומדלג על `id` שכבר קיים — הדילוג גם מרפא מאגר שכבר הוכפל אצל לקוח.

**כלל מחיקה — כל מחיקה מרובה חייבת ביטול.**
`flushCloudAfterDelete()` מוחק מהענן **מיד**, כך שמחיקה מוטעית היא סופית. בנוסף `deleteSelected()` מוחק כל נכס עם `selected!==false` — וכל העלאה מסומנת כברירת מחדל, כלומר הכפתור מוחק את *כל* המאגר. שלושת מסלולי המחיקה (`removeAsset`/`deleteSelected`/`deleteFolder`) שומרים את הנכסים ב-`_undoBuf` ומציגים "בטל ↩" ל-10 שנ'. גם: כפתור מחיקה לא יושב בתוך אזור לחיצה תמים בלי יעד מגע מלא.

## פורמטי תמונה — מה נתמך ואיך זה נבדק

**נתמכים (מאומת מול Chromium אמיתי):** JPEG (כולל progressive ו-CMYK), PNG (כולל 16-bit gray), GIF (כולל מונפש), WebP (כולל lossless+alpha), AVIF, HEIC/HEIF, BMP, TIFF (raw ו-LZW), ICO, SVG.

- **אין לסנן לפי `f.type`.** בוררי קבצים בנייד ובענן (Drive/OneDrive) מחזירים MIME ריק, ו-SVG מגיע כ-`text/xml`. הסינון (`looksLikeImage`) פוסל רק מה שידוע שאינו תמונה; הזיהוי האמיתי הוא `sniffMime()` לפי magic bytes ב-`loadImgFile`.
- **HEIC ו-TIFF מפוענחים ב-JS**, כי שום דפדפן (מלבד Safari) לא מפענח אותם: `heic2any` ו-`UTIF`.
- **הספריות האלה נטענות קודם מ-`vendor/` ורק אחר כך מ-CDN** (`loadLibOnce`). זה מכוון: HEIC הוא הפורמט של **כל** תמונה מאייפון, וכשהוא היה תלוי אך ורק ב-CDN חיצוני, כל חסימה/תקלת רשת הפכה את כל התמונות מאייפון לבלתי-ניתנות-להעלאה. הטעינה עצלה — הקבצים יורדים רק כשבאמת מגיע קובץ מהסוג הזה.
- `vendor/heic2any.min.js` (1.35MB, MIT) · `vendor/UTIF.js` (58KB, MIT).

### 🩺 בדיקה עצמית מובנית — לוח מנהל
`runUploadSelfTest()` (מקטע "בדיקה עצמית — העלאת תמונות"). מריצה דגימה אמיתית מכל פורמט (base64 מוטמע, ~4KB) דרך `looksLikeImage` + `loadImgFile` — **אותו צינור שהלקוח מריץ** — ומאמתת שהתקבל קנבס עם מידות אמיתיות. כל פורמט נבדק פעמיים: עם MIME ובלי MIME (המקרה שגרם לבאג). כולל בקרה שלילית — טקסט/PDF/וידאו שחייבים להידחות.

**למה זה שווה:** תמיכה בפורמטים משתנה בין מכשירים ודפדפנים. כשלקוח מדווח "התמונה לא נטענת", ההרצה הזו **על המכשיר שלו** היא התשובה, ולא ניחוש. כפתור "העתק דוח" מייצר טקסט עם ה-user-agent לשליחה בוואטסאפ.
